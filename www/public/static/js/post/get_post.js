function delete_btn(post_id, post_author) {
    if (post_author == $(".request-user").text()) {
        ...
        if ($("#post-delete").length == 0) {
            ...
            var post_detele_btn = $("<button />", {
                "class": "btn btn-default",
                "data-toggle": "modal",
                "href": "#post-delete-modal",
                "data-focus-on": "input:first"
            });
            var post_detele_span = $("<span />", {
                "class": "glyphicon glyphicon-remove",
                "aria-hidden": "true",
            });
            var post_detele_span_text = $("<span />", {
                "class": "post-delete-p",
                "text": "Delete this post"
            });
            post_detele.append(post_detele_btn);
            post_detele_btn.append(post_detele_span);
            post_detele_btn.append(post_detele_span_text);
            $(".post-details-body").after(post_detele);
        }
        // $("#post-details").on("click", "#really-delete-btn", function() {
...
function getPost(post_id, e, status) {
    return $.ajax({
        url: "/api/posts/" + post_id + "/",
        type: "GET",
        datatype: "json",
